...
--------------------
  ...
  set_groups [arrays|ranges]
    # set the groups of sequences, expects an array of arrays or ranges
    # usage, with a 16 sequences alignemnt:
    # >> set_groups [0..4, 5..9, 10..15]
    # >> set_groups [0..4, [5,7], 8..14, [15]]
  
  
Consensus coefficient ka
-------------------------
  con :preset_value
...
Aspecificity tollerance coefficient kb
---------------------------------------  
  asp :preset_value
...
---------------
  ...
  color_ranges n1, n2, n3, n4
    # defines the borders values of score ranges, for color masking
    # expects a list of 4 values, default is 0.5, 0.7, 0.8, 0.9
    # score ranges are :
    #   score < n1
    #   n1 <= score < n2
    #   n2 <= score < n3
